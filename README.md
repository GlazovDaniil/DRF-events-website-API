...
## *📊Диаграмма URLs📊* (устарела, ну да и черт с ней, есть swagger):
...
## *📜Задачи📜*:
 ...
 - ⛔ Предпологаемый возраст для участников мероприятий (нет, так как для разделения по группам есть теги)
 - ✅ Голосования в мероприятиях
 - ✅/⛔ Чат мероприятия и в принципе чаты (так как хостинг без asgi, а только синхронный сервер - чат фикция, которая работает при обновлении страницы,  не через веб-сокет)
 - ✅ Поиск мероприятий по сайту
 - ✅ Рекомендации мероприятий
 - ✅ Метод для добавления на мероприятие по QR-коду
...
*Изменения:*
- Добавлены фотографии профиля пользователя, банеры для мероприятий
- Доработана система прав
- Добавлены рекомендации для пользователей на основе их интересов (тегов)
- Добавлен поиск по сайту
- Различные исправления и оптимизация
- Проект подготовлен к выгрузке на хост
...
*Изменения:*
- БД перенесена на удаленный сервер
- СУБД измененио с SQLite на PostgreSQL
